...
void network_results(int g)
{
	...
	results_file.open("network_Results.txt", ios::out);
	results_file << "*********************************************************" << endl;
	results_file << "G:  " << endl;
	for (int i = 0; i < I; i++)
	{
...
#define X  99 
	for (int i = 0; i < N; i++)
	{
		...
		{
			act[i][0] = tmp_act[i][0] * 2 * (double(g) - 1) / double(g);
		}
...
void Play(int x, int g)
{
	...
	cout << " x= "<< x << "a1 = " << a1 << " D[x] = " << D[x] << " L_x = " << L_x << endl;
	...
	{
		tmp6 = A[i];
		payoff[tmp6] = g_payoff[tmp6] / double(D[tmp6]);
	}
}
...
double Update(int g, int T_L, int T_R, int T, int i_th)
{
	...
	for (t = 1; t <= T_L; t++)
	{
		...
		if (t % 1000000 == 0)
		{
			double temp_judgel_p = 0;
			double temp_judgel_q = 0;
			Out_act(t, g);
			Out_record(t, g);
			if ((convergence_of_judge1[0] != -1) && (convergence_of_judge1[1] != -1) && (convergence_of_judge2[0] != -1) && (convergence_of_judge2[1] != -1))
			{
				temp_judgel_p = fabs(convergence_of_judge2[0] - convergence_of_judge1[0]);
				temp_judgel_q = fabs(convergence_of_judge2[1] - convergence_of_judge1[1]);
				cout << "t= " << t << " p= " << temp_judgel_p << " q= " << temp_judgel_q << endl;
				for (int i = 0; i < 2; i++)
				{
					convergence_of_judge1[i] = -1;
					convergence_of_judge2[i] = -1;
				}
				if (temp_judgel_p <= 0.01 && temp_judgel_q <= 0.01)
				{
					if (t > 1000000) { break; }
				}
			}
	...
	for (t = T_L + 1; t <= T_L + T_R; t++)
	{
		...
		if (t == T_L + T_R)
		{
			...
			file << total_p / double(T_R) << " " << total_q / double(T_R) << endl;
...
int main() {
	...
	for (int g = start; g <= end; g++) {
		//read_one_file(i);
		string first_file_name = "E:\\程序\\超图上的多人最后通牒博弈\\scalefree_MUG1\\N=1000,G=";
